...
public class ControlleurAccueil {
		...
		valider.setOnAction((event) -> {
			...
			if (auto.isSelected()) {
				try{
					...
					if(value_robot <= 0 && value_obstacle <= 0){
						...
						afficher_fenetre_modale_d_erreur("La fonctionnalitÈ 'ParamÈtrer le nombre de robots et d'obstacles sur la simulation' n'est pas implÈmentÈe.");
					}
				}catch(Exception e){
			...
			if(conf.isSelected()){
				if(this.selectedFile == null){
					this.afficher_fenetre_modale_d_erreur("Aucun fichier configuration valide sÈlectionnÈ !");
				}else{
					afficher_fenetre_modale_d_erreur("La fonctionnalitÈ 'Charger des paramËtres de simulation depuis un fichier externe' n'est pas implÈmentÈe.");
				}
			}
		});
	}
	...
	private void definir_action_bouton_parcourir() {
		parcourir.setOnAction((event) -> {
			...
			if(this.selectedFile != null){
				nom_fichier.setText("Fichier sÈlectionnÈ : " + selectedFile.getName());
